...
<div class="d-flex align-items-center justify-content-between mb-3">
    ...
    <h4 class="card-title mb-0"><?= __('Posts') ?></h4>
    <?= $this->Html->link(__('New Post'), ['action' => 'add'], ['class' => 'btn btn-primary btn-rounded']) ?>
</div>

<p class="card-description">List of all available posts</p>
<div class="table-responsive">
    <table class="table table-striped">
        <thead>
            <tr>
                <th><?= $this->Paginator->sort('id') ?></th>
                <th><?= $this->Paginator->sort('blog_id', 'Blog') ?></th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <?= $post->hasValue('blog') 
                        ? $this->Html->link($post->blog->title, ['controller' => 'Blogs', 'action' => 'view', $post->blog->id]) 
                        : '' ?>
                </td>
                <td><?= h($post->title) ?></td>
                <td>
                    <?php
                    $cleanImage = basename($post->image);
                    $imagePath = WWW_ROOT . 'img' . DS . 'posts' . DS . $cleanImage;
                    $webPath = '/img/posts/' . $cleanImage;
                    ?>
                    <?php if (!empty($post->image) && file_exists($imagePath)): ?>
                        <img src="<?= $webPath ?>" alt="<?= h($post->title) ?>" width="100">
                    <?php else: ?>
                        <span class="text-muted">Image not found (<?= h($cleanImage) ?>)</span>
                    <?php endif; ?>
                </td>
                
                <td><label class="badge badge-info"><?= h($post->created->format('d M Y')) ?></label></td>
                <td><label class="badge badge-secondary"><?= h($post->modified->format('d M Y')) ?></label></td>
                <td class="actions">
                    <?= $this->Html->link(
                        '<i class="fa fa-eye"></i>',
                        ['action' => 'view', $post->id],
